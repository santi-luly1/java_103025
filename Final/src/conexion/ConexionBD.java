...
public class ConexionBD {
    private Connection getConexion() throws SQLException {
        return DriverManager.getConnection("jdbc:mariadb://localhost:3306/prendasDB", "root", "test");
    }

    private Prenda crearPrenda(ResultSet rs) throws SQLException {
        // FIXME
    	return new Prenda(rs.getInt("id_prenda"), rs.getString("nombre"), rs.getString("apellido"), rs.getInt("edad"), rs.getBoolean("estudiante"), rs.getString("sexo"), rs.getString("pais"), rs.getInt("paisIndex"));
    }
    ...
    public boolean insertarPrenda(Prenda p) throws SQLException {
        try (Connection con = getConexion();
             PreparedStatement pst = con.prepareStatement("INSERT INTO prenda(nombre, apellido, edad, estudiante, sexo, pais, paisIndex) VALUES(?, ?, ?, ?, ?, ?, ?)")) {
